...
const dashboard = {
  ...
  reviews: { open: 3, document: 1, gis: 1, record_parcel_link: 1, high: 1, medium: 1, assigned_to_me: 2 },
  ...
  jobs: { total: 6, by_status: [{ status: "COMPLETED", count: 5 }, { status: "FAILED", count: 1 }] },
  attention: { failed_jobs: 1, review_required_documents: 1, high_open_reviews: 1, ambiguous_record_parcel_links: 1, parcels_needing_review: 0 },
};

const reviewer = { id: "u", permissions: ["dashboard:read", "document:read", "geo:read", "review:read"], project_memberships: [{ project_id: "project", role: "REVIEWER" }] };
...
describe("DashboardPage", () => {
  ...
  it("keeps viewer navigation read-only and hides review workspace", async () => {
    renderPage({ ...reviewer, permissions: ["dashboard:read", "document:read", "geo:read"], project_memberships: [{ project_id: "project", role: "VIEWER" }] }, { ...dashboard, project_role: "VIEWER" });
  ...
  it("renders a stable empty-project state", async () => {
    renderPage(reviewer, {
      ...
      reviews: { open: 0, document: 0, gis: 0, record_parcel_link: 0, high: 0, medium: 0, assigned_to_me: 0 },
      ...
      attention: { failed_jobs: 0, review_required_documents: 0, high_open_reviews: 0, ambiguous_record_parcel_links: 0, parcels_needing_review: 0 },
    });
