...
export function buildingsForActiveImagery(buildings: GeoFeature[], imageryAssetId: string | null | undefined): GeoFeature[] {
  if (!imageryAssetId) return buildings;
  const activeReference = `imagery:${imageryAssetId}`;
  return buildings.filter((building) => {
    const isUnverifiedImageryCandidate = building.source === "AI_CANDIDATE"
      && building.status === "AI_PRELIMINARY"
      && building.verification_status === "UNVERIFIED"
      && building.source_reference?.startsWith("imagery:");
    return !isUnverifiedImageryCandidate || building.source_reference === activeReference;
  });
}
...
export function GisPage() {
  ...
  const empty = !data.parcels.length && !mapBuildings.length && !data.roads.length && !data.landUse.length;
