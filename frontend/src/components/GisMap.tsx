...
export function GisMap({ parcels, buildings, roads, landUse, topologyParcelIds, visibility, selectedParcelId, onParcelSelect, onFeatureSelect }: { parcels: Parcel[]; buildings: Building[]; roads: Road[]; landUse: LandUseFeature[]; topologyParcelIds: string[]; visibility: LayerVisibility; selectedParcelId: string | null; onParcelSelect: (id: string) => void; onFeatureSelect: (kind: MapFeatureKind, id: string) => void; }) {
  ...
  useEffect(() => {
    ...
    mapRef.current = map;
    map.on("load", () => {
      for (const [name, sourceData] of Object.entries(data)) map.addSource(name, { type: "geojson", data: sourceData as Parameters<maplibregl.GeoJSONSource["setData"]>[0] });
  ...
  useEffect(() => {
    ...
    for (const [name, sourceData] of Object.entries(data)) (map.getSource(name) as maplibregl.GeoJSONSource | undefined)?.setData(sourceData as Parameters<maplibregl.GeoJSONSource["setData"]>[0]);
    map.setPaintProperty("parcels-line", "line-color", ["case", ["==", ["get", "id"], selectedParcelId ?? ""], "#f7f3da", "#8f632f"]);
  }, [parcels, buildings, roads, landUse, topologyParcelIds, selectedParcelId]);
