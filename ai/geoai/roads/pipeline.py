...
def _metrics(model: RoadSegmenter, loader: DataLoader, device: torch.device) -> SegmentationMetrics:
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            tp, fp, fn = binary_confusion(probability, mask)
            true_positive, false_positive, false_negative = true_positive + tp, false_positive + fp, false_negative + fn
...
def train(config: RoadTrainingConfig) -> dict[str, object]:
    ...
    train_loader = _loader(RoadSegmentationDataset(config.dataset_root, "train", image_size=config.image_size, limit=config.limit), batch_size=config.batch_size, device=device, workers=workers, shuffle=True)
    validation_loader = _loader(RoadSegmentationDataset(config.dataset_root, "val", image_size=config.image_size, limit=config.limit), batch_size=config.batch_size, device=device, workers=workers, shuffle=False)
    model, start_epoch = create_model(RoadModelConfig(), device=device), 0
    ...
    try:
        for epoch in range(start_epoch, start_epoch + config.epochs):
            ...
            validation = _metrics(model, validation_loader, device).to_dict()
            history.append({"epoch": epoch, "train_loss": total / max(1, batches), "validation": validation})
            latest = save_checkpoint(output_dir / "latest.pt", model, optimizer_state=optimizer.state_dict(), epoch=epoch, training_config=asdict(config), validation=validation)
            if validation["iou"] > best_iou:
                best_iou = validation["iou"]
                best_checkpoint = save_checkpoint(output_dir / "best.pt", model, optimizer_state=optimizer.state_dict(), epoch=epoch, training_config=asdict(config), validation=validation)
    except torch.OutOfMemoryError as error:
        raise RuntimeError("CUDA out of memory. Retry with a smaller --batch-size; CPU fallback is disabled.") from error
    return {"run_id": run_id, "device": hardware.to_dict(), "checkpoint": str(best_checkpoint or latest), "history": history, "model_config": model.config.to_dict()}
...
def evaluate(dataset_root: str | Path, split: str, checkpoint: str | Path, *, device_request: str = "auto", batch_size: int = 2, num_workers: int | None = None, limit: int | None = None, image_size: int | None = None) -> dict[str, object]:
    ...
    metrics = _metrics(model, _loader(dataset, batch_size=batch_size, device=device, workers=choose_num_workers(num_workers), shuffle=False), device)
